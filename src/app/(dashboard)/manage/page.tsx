...
import { TrendingDownIcon, TrendingUpIcon, PlusSquareIcon, TrashIcon } from '@/constants/icons'
    ...
    <>
      <div className="border-b bg-card">
        <div className="container flex flex-wrap items-center justify-between gap-6 py-8">
          <div>
            <p className="text-3xl font-bode">Manage</p>
            <p className="text-muted-foreground">Manage you account settings and categories</p>
          </div>
        </div>
      </div>
...
function CategoryList({ type }: { type: TransactionType }) {
  ...
  const dataAvailable: boolean = categoriesQuery.data && categoriesQuery.data.length > 0;
      ...
      <Card>
        ...
        {!dataAvailable && (
          <div className="flex h-40 w-full flex-col items-center justify-center">
            <p>No <span className={cn("m-1", type === "income" ? "text-emerald-500" : "text-red-500")}>{type}</span> categories yet</p>
...
function CategoryCard({ category }: { category: Category }) {
  return (
    <div className="flex border-separate flex-col justify-beween rounded-md border shadow-md shadow-black/[0.1] dark:shadow-white/[0.1]">
